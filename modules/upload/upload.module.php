...
class ModuleUploadUpload extends Module{
	
	private $filepath = './uploads/';     //指定上传文件保存的路径
	private $filesubdir = '';     //指定上传文件保存的子目录
	...
	public function init($options = array()){		
		...
		$filesubdir = $this->filesubdir ? '/'.trim($this->filesubdir,'/') : '';
		$this->setoption('filesubdir',$filesubdir);
		$this->setoption('filepath', $this->config->get('upload_file').$this->filesubdir.'/'.date('Ym/d'));
		$this->setoption('maxsize', $this->base->get_config('upload_maxsize')*1024);
	}
	...
	public function getnewfileinfo(){
		$arr = array(
			...
			'filepath' => '/'.$this->config->get('upload_file').$this->filesubdir.'/'.date('Ym/d/'),
			'fileurl' =>  $this->config->get('uploadhost') . WEBDIRNAME . '/'.$this->config->get('upload_file').$this->filesubdir.'/'.date('Ym/d/'),
	...
	public function getnewfilename($file_path = true){
		if($file_path)
			return $this->config->get('uploadhost') . WEBDIRNAME . $this->config->get('upload_file').$this->filesubdir.'/'.date('Ym/d/').$this->newfilename;
		else
