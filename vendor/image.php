...
/**
 * 使用方法
 * $image = new Image(1,$sourcepath);
 * if(!is_dir(dirname($thumbpath))) mkdir(dirname($thumbpath),0777,true); 
    $image->thumb($width, $height, Image::IMAGE_THUMB_CENTER)->save($thumbpath, null, 100);
    ## 直接显示图片
    $imginfo = $image->getimginfo();
    header('HTTP/1.1 200 OK',true, 200);
    header("Last-Modified: " . gmdate("D, d M Y H:i:s") . "GMT");
    header('Content-Type:'.$image->mime());
    $funcs = 'image'.$image->type();
    $funcs($image->getimgobject());
 */
class Image{
    /* 驱动相关常量定义 */
